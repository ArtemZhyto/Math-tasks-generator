...
export const evaluateNumericPart = (template: string, variables: Record<string, number>): number | string => {
  ...
  //C: Якщо вираз містить строкові функції (greater, less, equal, compare, isPrime), то повертаємо строковий результат
  ...
  if (hasStringFunctions) {
    try {
      ...
      logger.info('NUMERIC_PART', 'Строковий результат:', stringResult)
			...
			if (typeof stringResult === 'number') {
				return stringResult
			}

			const numericMatch = stringResult.match(/(-?\d+[.,]?\d*)/)
