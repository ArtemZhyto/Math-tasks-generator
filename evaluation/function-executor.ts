...
export const isStringLiteral = (arg: string): boolean => {
  const result = (arg.startsWith("'") && arg.endsWith("'")) || 
         (arg.startsWith('"') && arg.endsWith('"'))
  logger.info('STRING_CHECK', 'Перевірка строкового літерала:', arg, 'Результат:', result)
  return result
}
...
export const executeFunction = (func: string, args: string[]): string => {
  ...
  const cleanArgs = args.map(arg => 
    isStringLiteral(arg) ? arg.slice(1, -1) : arg
  )
  
  const result = executeMathFunction(func, cleanArgs)
...
const handleConcat = (args: string[]): string => {
  logger.info('CONCAT', 'Спеціальна обробка concat')
  
  const processedArgs = args.map(arg => {
    if (isStringLiteral(arg)) {
      //C: Видалення лапок з строкових літералів
      //C: Remove quotes from string literals
      return arg.slice(1, -1)
    }
    
    //C: Збереження ком у числах для десяткових значень
    //C: Preserve commas in numbers for decimal values
    if (arg.includes(',')) {
      return arg
    }
    
    //C: Конвертація числових рядків у числа та назад для нормалізації
    //C: Convert numeric strings to numbers and back for normalization
    const num = Number(arg)
    return !isNaN(num) ? num.toString() : arg
  })
  
  return MathFunctions.concat(...processedArgs)
}
