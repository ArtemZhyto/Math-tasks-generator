...
export const parseArguments = (argsStr: string): string[] => {
  ...
  for (let i = 0; i < argsStr.length; i++) {
    ...
    if ((char === "'" || char === '"') && !inQuotes) {
      ...
      if (i > 0 && i < argsStr.length - 1 && 
          /\d/.test(argsStr[i-1]) && /\d/.test(argsStr[i+1])) {
        //C: Збереження коми у десяткових числах
        //C: Preserve comma in decimal numbers
        current += char
      } else {
        //C: Додавання завершеного аргументу до списку
        //C: Add completed argument to list
  ...
  //C: Додавання останнього аргументу якщо він є
  //C: Add last argument if exists
  if (current.trim()) {
    args.push(current.trim())
  }
