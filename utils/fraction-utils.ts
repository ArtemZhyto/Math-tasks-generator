...
export const checkIfNegativeFraction = (fraction: string): boolean => {
  if (fraction.includes('-')) {
    return true
  }

  const negativeNumeratorMatch = fraction.match(/<sup>(-?\d+)<\/sup>/)

	if (negativeNumeratorMatch && negativeNumeratorMatch[1].startsWith('-')) {
    return true
  }

  const textNegativeMatch = fraction.match(/(-?\d+)\s*\/\s*(-?\d+)/)

	if (textNegativeMatch && textNegativeMatch[1].startsWith('-')) {
    return true
  }
